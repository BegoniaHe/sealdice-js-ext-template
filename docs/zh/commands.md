...
| `./sealw init --preset single-target --directory <path> --target <id>` | 在不存在的新目录生成只保留一个精确 target 的精简项目。                             |
...
## 原生 TypeScript 测试与会话 helper

`test` 会发现 `tests/` 下的 `*.test.mjs` 和 `*.test.ts`。Node 26 直接执行后者，因此它们必须使用可擦除的
TypeScript 语法、ESM 与显式 `.ts` 相对导入；模板会在不改变插件 runtime 类型边界的独立 tsconfig 中检查这些测试。

`tests/support/mock-host.mjs` 提供 profile 驱动的 mock，并包含 `messages`、`setConfig()`、`clock`、`runTask()` 和
`assertNoActiveTasks()`。会话插件应使用 `clock.advanceBy()` 控制时间，并在会话结束时断言无活动任务或待处理计时器。

## 制品策略

`release.artifactPolicy.forbiddenPaths` 和 `forbiddenExtensions` 在 `.sealpack` staging 后检查最终归档路径。路径支持
`*`、`**` 与 `?`；后缀使用小写形式，例如 `.png`。这适合禁止本地 assets 或图片。若要限制资源安装等行为，须结合权限与
runtime policy，因为行为不一定能从归档路径推断。

## Userscript 时间戳

每个 bundle 的 userscript header 都会写入 Unix 秒级 `@timestamp`。SealDice 将它作为脚本的显示更新时间，
并不用于加载或缓存失效。需要字节级可复现的发布时请设置 `SOURCE_DATE_EPOCH`；GitHub Release 工作流会使用当前
checkout 提交的时间。
