// Package analyzer statically reads the SealDice Go source without importing it.
package analyzer
...
// Result is a deterministic, intentionally low-level description of JsInit exports.
type Result struct {
...
// Entry describes a member set on seal or one of its nested objects.
type Entry struct {
...
// Type preserves Go source metadata which needs a separate TypeScript override.
type Type struct {
	Kind   string  `json:"kind"`
...
// Field is a Go struct field and its jsbind spelling when one is known.
type Field struct {
...
func Scan(core string) (Result, error) {
	...
	fingerprint, err := fingerprintSource(root)
	if err != nil {
		return Result{}, err
	}
	fset := token.NewFileSet()
	packages, err := parser.ParseDir(fset, diceDirectory, func(file os.FileInfo) bool {
		return strings.HasSuffix(file.Name(), ".go") && !strings.HasSuffix(file.Name(), "_test.go")
	...
	for _, file := range pkg.Files {
		for _, declaration := range file.Decls {
			...
			case *ast.FuncDecl:
				// JsInit binds package-level functions. Methods with the same name
				// (for example platform adapter methods) are not valid replacements.
				if node.Recv == nil {
					funcs[node.Name.Name] = node
				}
	...
	entries := scanJsInit(fset, jsInit, funcs)
...
func scanJsInit(fset *token.FileSet, jsInit *ast.FuncDecl, funcs map[string]*ast.FuncDecl) []Entry {
	type setCall struct {
		receiver string
		name     string
		value    ast.Expr
	...
	ast.Inspect(jsInit.Body, func(node ast.Node) bool {
		...
		entry := describeSetValue(fset, call.Args[1], funcs)
		entry.Source = sourceLocation(fset, call.Pos())
	...
	for progress := true; progress; {
		...
		for _, call := range calls {
			...
			if entry.Kind == "object" {
				if ident, ok := call.value.(*ast.Ident); ok && paths[ident.Name] != entry.Path {
					paths[ident.Name] = entry.Path
					progress = true
				}
			}
		}
...
func describeSetValue(fset *token.FileSet, expression ast.Expr, funcs map[string]*ast.FuncDecl) Entry {
	switch value := expression.(type) {
	case *ast.FuncLit:
		return entryFromFuncType(fset, value.Type)
	case *ast.Ident:
		if declaration, ok := funcs[value.Name]; ok {
	...
	case *ast.CallExpr:
		...
		if ident, ok := value.Fun.(*ast.Ident); ok {
			if declaration, found := funcs[ident.Name]; found && declaration.Type.Results != nil && len(declaration.Type.Results.List) > 0 {
...
func entryFromFuncType(fset *token.FileSet, function *ast.FuncType) Entry {
	signature := functionSignature(fset, function)
	entry := Entry{Arity: fieldArity(function.Params), GoSignature: signature, Kind: "function"}
...
// functionSignature retains the exported Go type while removing source-only
// parameter names. That makes static signatures comparable to reflect.Type.
...
func sourceLocation(fset *token.FileSet, position token.Pos) string {
	info := fset.PositionFor(position, false)
	return filepath.Base(info.Filename) + ":" + strconv.Itoa(info.Line)
}
...
// WriteAtomic writes a generated report without exposing partially-written files.
func WriteAtomic(destination string, contents []byte) error {
	directory := filepath.Dir(destination)
	if err := os.MkdirAll(directory, 0o755); err != nil {
		return err
	}
	temporary, err := os.CreateTemp(directory, ".seal-api-scan-*")
	if err != nil {
		return err
	}
